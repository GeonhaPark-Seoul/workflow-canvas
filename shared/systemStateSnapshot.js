...
import { SOURCE_TWIN_SCHEMA_VERSION, SOURCE_TWIN_SOURCE_ID } from './sourceTwin.js'

// The wire values retain their deployed source-twin names for compatibility.
// Ownership belongs to the cross-engine state/safe-operation workflow, not to
// the Source Lens analysis engine.
...
export function createSourceTwinSnapshot({
  ...
  if (manifest?.schemaVersion !== SOURCE_TWIN_SCHEMA_VERSION || !manifest?.id) {
    throw new Error('유효한 소스 분석 manifest가 필요합니다.')
  }
