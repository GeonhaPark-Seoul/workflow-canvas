...
캔버스는 서로 다른 작업 관점을 한 평면에 억지로 섞지 않는다.

> **노드는 코드·데이터·요청이 머물거나 실행되는 지속적인 장소 또는 독립
> 시스템이다.**

Asset이 곧 노드는 아니다. Job, Build, Deployment, Request, Commit 같은 실행·사건은
대상 노드의 파츠·흐름·타임라인으로 표현한다. 사람과 AI의 활동은 Actor·Presence
표시로, 환경·계정·신뢰영역은 필요에 따라 그룹이나 오버레이로 표현한다.

**층(Layer) = 특정 사용자 질문과 Work를 위한 화면.** 모든 캔버스에서 사용자가
만들고, 이름 짓고, 순서를 정하고, 노드를 배정할 수 있다. 시스템 지도에도 보호된
공식 층수·순서·종류별 자동 배정표를 두지 않는다. 비개발자용 화면과 개발자용
화면이 함께 존재할 수 있고, 사용자가 요청한 화면을 추가한다.

같은 실제 대상을 여러 화면에서 보여줄 때는 하나의 Canonical Asset을 여러 View
Node가 참조한다. 실제 상태·근거·권한·관계는 `assetId`를 통해 동기화하고,
위치·크기·접힘·표시 파츠·화면별 그룹은 각 화면에 따로 저장한다.

**오버레이(Overlay) = 여러 화면을 가로질러 칠하는 관점:** 보안·신뢰, 데이터
흐름, 운영 상태, 변경, 비용, 소유권, 위험, 근거 수준 등이 해당한다. 보안은
Pipeline 하나에 갇히지 않으며 필요하면 별도 보안 화면도 가진다.

**구현 원칙:**
- 레이어는 저장 뷰와 전환기, 명시적 노드 배정, 투영, redaction-safe 포털이라는
  일반 기능으로 유지한다.
- 시스템 종류나 Trust Zone만 보고 레이어를 자동 결정하지 않는다.
- 동일 Asset의 여러 View Node 동기화는 `assetId` 기반으로 구현한다.
- 3D는 같은 데이터를 그리는 후속 렌더러다. 여러 화면에 걸친 동일 Asset은
  직육면체·원기둥·축 같은 하나의 관통 형상으로 표현할 수 있다.
- 화면은 한 번에 하나씩 목적, 관측 Workflow, 실행 동작, 노드와 비노드 표현을
  사용자와 확정한 뒤 구현한다.

**기능 정보의 표현.** 기능이라는 이유만으로 노드가 되지 않는다:

| 판정 | 표현 | 예 |
|---|---|---|
| 기능을 수행하는 지속 장소 또는 독립 시스템 | 노드 | 실행 중인 앱·서비스, 인증 시스템 |
| 어떤 Asset이 제공하는 능력 | 소유 노드의 파츠 또는 Workflow 단계 | 로그인, 공유·초대, Git 동기화 |
| 일시적인 기능 실행 | 흐름·Run·Event | 로그인 요청, 동기화 실행 |
| 상태 관리가 필요 없는 사실 | 속성·관측값 | 모바일 지원, PWA 설치 가능 |

Source Lens의 제품 영역 × 하위 시스템 분류는 화면의 그룹·파츠·탐색 계층 후보로
재사용한다. 어떤 화면에 어떻게 표현할지는 자동 승격하지 않고 화면별로 판정한다.
...
- **사용자 레이어**: 레이어 생성·이름·순서·삭제, 명시적 layerOverride, 화면별
  투영과 수직 포털(일시적, redaction-안전 카운트). 보호된 공식 레이어와 종류별
  자동 배정은 폐기 (§2.4)
...
### 다음 제품 순서 (단기, 확정)
1. ~~**레이어 표현 기반**~~ ✅ 완료 — 저장 뷰, 전환기, 명시적 layerOverride,
   redaction-안전 수직 포털. 초기에 함께 들어간 고정 L1~L4 프리셋과 종류별 자동
   배정은 2026-07-24 폐기하고 일반 표현 기능만 유지한다.
1-2. **동일 Asset의 다중 화면 표현** — 여러 View Node가 같은 `assetId`의 실제
   상태·근거·권한·관계를 동기화하고 화면별 배치는 독립적으로 보존한다.
2. ~~**기능 경계 분석**~~ ✅ 분석 엔진 완료 (배치 B, 커밋 `24a36e6`) —
   Feature Boundary Resolver의 근거 수집과 경계 분석은 재사용한다. 분석 결과를
   노드로 만들지 파츠·흐름으로 만들지는 각 화면 검토에서 새 노드 원칙으로 판정한다.
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| 0.16.0 | 2026-07-24 | 시스템 지도의 보호된 L1~L4와 제안된 고정 L1~L5 계약을 폐기했다. 노드를 “코드·데이터·요청이 머물거나 실행되는 지속적인 장소 또는 독립 시스템”으로 확정하고, 레이어를 고정 온톨로지가 아닌 목적별 화면으로 재정의했다. 같은 Canonical Asset을 여러 View Node가 참조해 실제 상태를 동기화하고 화면 배치는 독립 보존하는 방향, 비노드 표현 문법, 보안의 횡단 범위, 한 화면씩 검토하는 절차를 반영했다. |
