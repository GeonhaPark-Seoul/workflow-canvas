...
|---|---|---|---|---|
| 지도 화면 | `@xyflow/react` 유지 | 다른 캔버스 렌더러 | 유지 | 대규모 지도 성능이나 접근성 한계가 측정될 때 |
...
| 지도 저장 | Supabase PostgreSQL의 정규 자료와 캔버스 JSON 유지 | 관계 탐색 전용 DB | 후보 | 다중 hop 질의와 공격 경로 분석이 PostgreSQL 모델의 측정된 한계를 넘을 때 |
...
제품·엔진 구성층은 새 지도 라이브러리나 권한 서비스를 도입하지 않고 Engine Registry,
Capability Mapper, React Flow와 Asset 원장의 현행 wire 스키마 코드 이름인 `TwinBuild`
계약으로 구현한다. 목적은 제품 엔진을 눈에 보이게 하는 것이며 새로운 레이아웃 또는
권한 문제를 푸는 작업이 아니기 때문이다.
