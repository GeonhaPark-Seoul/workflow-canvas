...
SL-4는 import/call 관계 지도, React 렌더·props, UI 이벤트, API 라우트, MCP 도구를 서로 다른 `CODE` 근거로 보존한다. 코드 브라우저와 Component 파츠는 실행 기록이 없는 관계를 `declared`로 표시하고, 동적 dispatch는 `unknown`으로 남긴다.
...
- 대형 지도 배치, 산출물 위조 방지, 라벨 sanitize, 번들 크기를 별도로 관리해야 한다.
- Mermaid 문자열을 Source of Truth로 삼으면 캔버스 관계 근거와 두 개의 실체가 생긴다.
- 현재 캔버스 엔진과 다른 호버·선택·층·오버레이 문법을 새로 학습해야 한다.

## 권고안

- SL-5에서 진입점 하나의 작은 부분 지도만 서버에서 sanitize해 읽기 전용 Mermaid 미리보기로 만드는 파일럿을 검토한다.
