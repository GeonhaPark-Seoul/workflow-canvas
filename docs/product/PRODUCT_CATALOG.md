...
| 표시 이름 | 사용자 가치 | 현재 상태 |
|---|---|---|
| Twin Core | 여러 시스템의 근거를 공통 디지털 트윈으로 변환하고 현재 지도와 대조 | 알파 |
| Create Graph | 여러 노드와 관계를 검증·배치해 실제 캔버스로 생성 | 알파 |
| Source Lens | 코드를 기능 묶음과 쉬운 설명, 개발자 근거로 탐색 | 알파 |
...
1. 이번 제품·엔진 구성층을 실제 시스템 지도에 적용하고 화면 검증
2. Workflow Canvas 전용 Source Profile을 공통 계약 뒤로 분리
3. Python/FastAPI 등 두 번째 실제 소프트웨어 스택으로 범용성 검증
4. 비개발자가 연결 범위와 권한을 이해하는 시스템 가져오기 흐름
5. 테스트, 상태 확인, 비강제 Git 동기화 중심의 제한 조작 확대
