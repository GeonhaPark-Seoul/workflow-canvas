...
`Source Twin`과 `Twin Adapter`는 배포 호환을 위해 유지하는 레거시 현행 코드·wire 이름이며,
새로운 내부 메커니즘 이름이 아니다. 이들이 전달하는 제품 개념은 Asset 원장으로 설명한다.

- Source Analysis Pipeline: 허용된 파일을 찾고 지원되는 parser로 구조 근거를 추출한다.
- Source Profile Registry: 저장소 근거와 일치하는 프로필 하나를 결정적으로 선택한다.
- Source Profile: 제품별 이름, 분류와 파일 역할을 선언한다.
- Functional Context Resolver: 프로필이 기능 경계 능력을 선언한 경우에만 실제 파일이
  존재하는 영역·서브시스템 이름을 G10-0 기능 어휘 후보로 사용한다.
- Feature Boundary Resolver: 프로필의 3등급 판정과 실제 코드·DB 근거를 대조한다.
- 레거시 현행 Artifact 이름인 `Source Twin manifest`: 선택된 프로필 ID·버전·근거와 분석 결과를 함께 보존한다.
- 레거시 현행 코드명인 `Twin Adapter`: 코드 외의 DB, 배포, 운영 상태와 조작 능력까지 Asset 원장으로 변환한다.
...
Workflow Canvas Profile 0.9는 `functional-context-bootstrap` capability를 선언한다. 이
capability는 G10-0 실행 자체를 켜고 끄는 플러그인 권한이 아니라, 프로필이 현재
`FunctionalContextPack` 준비 단계와 호환됨을 알리는 지원 수준이다. G10-0은
`feature-boundary-classification` capability가 없는 범용 프로필의 영역·서브시스템 라벨을
기능 어휘로 추측하지 않는다. README·기획 문서, UI 문구·화면 경로·API·DB·테스트·정적
Flow의 실제 근거는 Profile 라벨과 별도로 수집한다.
...
판정은 Source Profile의 데이터 선언이다. 공통 Source Lens 코어에는 Workflow Canvas 이름이나 노드 ID를 하드코딩하지 않는다. `implementationRules`는 위에서 아래로 평가해 파일마다 처음 일치한 레거시 `Twin Adapter` 대상 하나를 사용한다. `dataBindings`는 레거시 wire 이름인 `Source Twin`의 DB Asset ID를 대상 Asset 원장의 Asset ID에 연결한다.

`feature-asset`은 해당 영역·하위 시스템의 실제 파일 근거와 구현 대상 근거가 모두 있을 때만 지도 반영 후보가 된다. `capability`는 실제 파일 근거와 소유 기능 Asset이 있어야 한다. DB 관계는 레거시 `Source Twin` Artifact가 `read` 또는 `write`로 확인한 참조만 `reads`/`writes` 후보가 되며 SQL 선언만 있는 `declares`는 사용하지 않는다.

판정 결과는 자동으로 캔버스를 바꾸지 않는다. 레거시 현행 코드명인 `Twin Adapter`가 기존 Reconciliation 경계에서 미리보기 → 사용자 승인 → 반영 순서의 Proposal로 바꾼다. Feature Asset은 `declared`이며 코드 근거만으로 LIVE를 암시하지 않는다.

Source Lens의 표현 규칙과 판정 코드도 레거시 `Source Twin` Artifact의 분석 대상이다. 따라서 판정기가 바뀌면 Source Lens 버전·근거와 자기 시스템 지도의 변경 검토안에 다시 나타난다. 이 자기반영 구조는 유지하되, 구체적인 세분화 수준은 프로필 버전으로 바꿀 수 있다.

## 코드 Asset 계층 v1

코드 탐색은 `제품 영역 → 서브시스템 → Component → 모듈 → 코드 단위` 계층을 사용한다. 현재 모듈은 파일과 함수이며 둘 다 Asset 후보지만, 캔버스 노드 자동 반영은 금지하고 `proposal-required`로 기록한다.
...
6. 실제 DB·배포·운영·조작이 필요하면 레거시 현행 코드명인 `Twin Adapter`를 연결한다.
...
이번 버전은 Workflow Canvas Profile 0.9의 기능 표현 경계와 G10-0 capability, 두 번째
FastAPI 참조 프로필에서 같은 판정기의 재사용까지 포함한다. FastAPI는 여전히 파일 구조
수준이며 실제 Python 함수·import·호출 분석은 표준 library 또는 검증된 parser를 비교하고
사용자 승인을 받은 뒤 도입한다. 그 전까지 Python 결과를 `parsed`나 LIVE로 승격하지 않는다.
