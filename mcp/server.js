...
## 노드 종류
- stage (단계 노드): 작업·항목·개념의 기본 블록. stageTypeIdx로 **계층(hierarchy level)**을 분류합니다. 종류 목록은 **캔버스마다 독립적**이며 사용자가 직접 정의할 수 있으니, 캔버스를 바꿀 때마다 get_stage_types로 그 캔버스의 현재 목록을 다시 확인하세요.
- memo (메모 노드): 비고·참조·참고사항 등 **보조 설명 전용** 노드. **계층 구조의 구성원이 될 수 없다** — 상위/중위/하위 어느 계층으로도 쓰지 말 것. 반드시 설명할 대상 stage 노드 옆에 꼬리표처럼 붙이고 그 stage와 점선으로 연결. (점선은 자동 적용됨)
...
- 주제·단계·산출물 → stage 노드 (stageTypeIdx로 계층(깊이) 구분)
- 비고·팁·주의사항·참고 → memo 노드 + 대상 stage로 edge (memo는 계층이 아니라 주석)
- 순서·인과·의존 관계 → edges
...
## stageTypeIdx 사용 지침
stageTypeIdx는 stage 노드가 속한 **계층(hierarchy level)**을 나타내는 index입니다 (get_stage_types 결과 배열의 순서, 해당 캔버스 기준).
- 방사형 구조(radial): 깊이별 계층 — 핵심 주제(깊이0) / 추진 방안(깊이1) / 세부 과제(깊이2) 등
- 흐름형 구조(right/down 등): 진행 단계 — 기획→개발→검토→배포→완료 순서가 곧 계층이다
- **주제·팀·중요도 같은 카테고리 구분에는 쓰지 말 것.** stageTypeIdx는 "어느 계층에 속하는가"를 나타내며, 색은 계층에 따라오는 부수 효과일 뿐이다.
...
- **계층(깊이) 기준으로만 종류를 부여할 것. 가지(branch)별로 종류를 나누는 것은 금지.**
  - ❌ 잘못: 방안①=초록(종류0), 방안②=빨강(종류1), 방안③=파랑(종류2) — 같은 깊이인데 가지마다 색이 달라짐
  - ✅ 올바름: 중심 주제=종류A, 모든 방안(깊이1)=종류B, 모든 세부 과제(깊이2)=종류C — 깊이별 통일
- 종류 이름은 내용에 맞게 rename_stage_type으로 바꿀 것. 마땅한 이름이 없으면 '핵심 주제', '1단계', '2단계'… 또는 '최상위/상위/하위'.
- layout:'radial'을 쓰면 4방향 배치·하위 계층 연결점 고정·메모 꼬리표가 자동 처리됨.
...
export function buildServer(getUserId) {
  ...
  server.registerTool('create_graph', {
    description:
      ...
      '하위 계층이 바깥으로 뻗음, 하위 계층은 부모의 한 연결점에서 나옴, 메모는 꼬리표처럼 옆에), ' +
      ...
      '【계층 색상 규칙 — 계층적 주제에 create_graph를 쓸 때 필수】\n' +
      '종류는 깊이(레벨) 기준으로만 부여. 가지(branch)별로 다른 종류를 주는 것은 금지.\n' +
      '❌ 잘못: 방안①=종류0, 방안②=종류1, 방안③=종류2 (같은 깊이인데 가지별 색 구분)\n' +
      '✅ 올바름: 중심 주제=종류A, 모든 방안=종류B, 모든 세부 과제=종류C (깊이별 통일)\n' +
      'rename_stage_type으로 종류 이름을 "핵심 주제/추진 방안/세부 과제"처럼 의미 있게 변경할 것.\n\n' +
