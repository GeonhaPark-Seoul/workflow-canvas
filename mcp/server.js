...
const SYSTEM_NODE_FIELDS = {
  ...
  responsibility: z.string().optional().describe('이 Asset이 책임지는 일과 책임지지 않는 경계 (HTML 지원).'),
...
1. 자료 재구성 → 2. 노드 제작 → 3. Draw Map을 통한 지도 배치 (한 번 호출)
...
- 순서: get_stage_types 확인 → rename_stage_type/create_stage_type으로 계층 이름 정리 → Draw Map 한 번으로 전체 생성

## 3. Draw Map (지도 배치 Engine)
- Draw Map은 배치·연결선·그룹만 담당하며 온톨로지·사실 판정과 무관한 순수 가시화 Engine입니다.
- Draw Map의 wire 이름은 create_graph다.
...
### 대량 작업 (중요)
노드를 2개 이상 만들 때는 create_node를 반복 호출하지 말고 반드시 Draw Map을 한 번 호출하세요.
...
새 노드 여럿과 연결선을 함께 만들 때는 Draw Map을 사용하세요.
...
export function buildServer(getUserId, { workshopStore = defaultWorkshopStore } = {}) {
  ...
  server.registerTool('create_node', {
    description:
      ...
      'type=memo → 메모 노드(header/text), type=system → 시스템 Asset(label + 온톨로지 필드).\n\n' +
      '【단계 노드(stage)란】\n' +
      ...
      '【시스템 노드(system)란】\n' +
      '앱·API·서버 함수·DB·테이블·인증·정책·배포 환경 같은 시스템 Asset을 표현한다. ' +
      '생성 직후 상태는 항상 사람이 선언한 설계이며, MCP 입력만으로 이 Asset을 정식 검증 상태인 LIVE로 만들 수 없다. ' +
      ...
      '⚠️ 노드를 2개 이상 만들 계획이면 이 도구를 반복 호출하지 말고 Draw Map 하나로 만들 것 (훨씬 빠름).',
    inputSchema: {
      canvas_id: z.string(),
  ...
  server.registerTool('create_graph', {
    description:
      'Draw Map은 노드와 연결선으로 이루어진 지도 전체를 **한 번의 호출**로 생성합니다. ' +
      '배치·연결선·그룹만 담당하며 온톨로지·사실 판정과 무관한 순수 가시화 Engine입니다. ' +
      'Draw Map의 wire 이름은 create_graph다.\n' +
  ...
  server.registerTool('create_edge', {
    description:
      ...
      '같은 방향·같은 relationType만 중복으로 거부됩니다. 새 노드들과 함께 여러 연결을 만들 때는 Draw Map을 쓸 것.',
    inputSchema: {
      canvas_id: z.string(),
  ...
  }, g(async (userId, a) => okh(await store.createCanvas(userId, a.name), '이제 Draw Map으로 내용을 채우세요.')))
  ...
  server.registerTool('inspect_source_twin', {
    ...
    inputSchema: {
      ...
      limit: z.number().int().min(1).max(500).optional().describe('반환할 Asset 최대 개수. 기본 200.'),
    },
  ...
  server.registerTool('compare_source_twin_snapshots', {
    description:
      '제품 소유자 전용 읽기 전용 비교: 두 통합 상태 스냅샷 사이의 코드 Asset, DB 선언, 배포, 운영 지표, ' +
