...
- 사람용 제품 기획 정본: [`docs/MASTER.md`](./docs/MASTER.md)
- 온보딩 전 Project Master·상대 AI 준비: [`Starting Protocol`](./docs/protocols/STARTING_PROTOCOL.md)
- 실제 시스템 근거 분석 전체 규약: [`System Onboarding Protocol`](./docs/protocols/SYSTEM_ONBOARDING_PROTOCOL.md)
- AI Context Gate Engine 상세 정본: [`AI Context Gate Master`](./docs/engines/AI_CONTEXT_GATE_MASTER.md)
- Source Lens Engine 상세 정본: [`Source Lens Master`](./docs/engines/SOURCE_LENS_MASTER.md)
- 교차 Engine Workflow·UI Projection 카탈로그: [`Workflow Catalog`](./docs/protocols/WORKFLOW_CATALOG.md)
- 개발 AI 작업 지휘·읽기 라우터: [`docs/AI_MASTER.md`](./docs/AI_MASTER.md)
...
`Asset`, `관계`, `미모델 자원` 변경을 앱 안의 검토 패널로 보여줍니다. 현재 데이터는
...
제품 소유자는 Workflow Canvas 시스템 지도에서 관련 Asset을 선택해 그 Asset의 근거를 엽니다.
...
말합니다. 코드 저장소 보기 안에서만 `제품 영역 → 서브시스템 → Component → 모듈 → 코드 파츠` 순서로 묶어
Asset Core 같은 기능의 시작과 범위를 먼저 볼 수 있고, 하위 시스템은 기본적으로
...
- Source Lens 단일 공개 진입점: [`scripts/source-lens-engine.mjs`](./scripts/source-lens-engine.mjs)
- 내부 AST·SQL 분석기: [`scripts/source-twin-scanner.mjs`](./scripts/source-twin-scanner.mjs)
- Source Analysis 읽기 모델: [`shared/sourceTwin.js`](./shared/sourceTwin.js)
- 교차 Engine 상태 Snapshot 코어: [`shared/systemStateSnapshot.js`](./shared/systemStateSnapshot.js)
...
- 상용화·보안 부채 장부: [`Technical Debt Ledger`](./docs/governance/TECHNICAL_DEBT.md)
- Asset 원장 문서 안내와 과거 방향 참고: [`Asset Base documentation`](./docs/twin/README.md), [`Legacy Engine Roadmap`](./docs/twin/archive/TWIN_ENGINE_ROADMAP.md)
...
`제품·엔진 구성층 (논리)`이 있습니다. `Asset Core`, `Draw Map`, `Source Lens`,
...
시스템 가져오기 전에는 [`Starting Protocol`](./docs/protocols/STARTING_PROTOCOL.md)이 사용자가 소유하는
단일 Project Master와 상대 개발 AI의 유지 규칙을 준비합니다. 최초 가동 시작점과
웹·로컬·IDE 등 최종 앱 형태는 아직 미정입니다. 그다음 다른 소프트웨어를 실제 근거와
연결하는 전체 순서·승인·완료 조건을
[`시스템 온보딩 프로토콜`](./docs/protocols/SYSTEM_ONBOARDING_PROTOCOL.md)이라고 부릅니다.
provider 형식을 Stage 계약에 맞추는 구현은 `Adapter`, 로컬·외부 경계를 넘어 통신하는 통로는
`Connector`, 근거를 공통 Asset 원장으로 변환하는 과정은 `Asset Build`입니다.
노드는 Asset, 파츠는 능력과 입출력 포트, 연결선은 계약과 흐름을 나타냅니다. 로컬 기기,
인트라넷, 사설·공개 클라우드, 인터넷, 외부 SaaS는 신뢰 구역으로 구분하며 구역을 넘는
관계는 인증·권한·데이터 종류가 명시된 게이트웨이를 거쳐야 합니다. 장기 Twin 모델은
[`Legacy Twin Engine Roadmap`](./docs/twin/archive/TWIN_ENGINE_ROADMAP.md)를 참고하되, 단계별 현재/계획·
조작 범위는 시스템 온보딩 프로토콜을 기준으로 합니다.
...
| `get_canvas` | 특정 캔버스의 노드/연결선 데이터 (시스템 Asset·관계 의미·근거·단계 종류 포함) |
| **`create_graph`** | **Draw Map의 wire 이름은 `create_graph`다. 노드+연결선 지도를 한 번에 생성(자동 배치 지원) — 대량 작업의 기본 도구** |
...
소스 분석의 GitHub push 이벤트와 통합 상태 이력을 사용하려면
...
| `WORKFLOW_CANVAS_OWNER_USER_ID` | 선택 | 내부 시스템 지도와 소스 분석 조회·이력을 허용할 제품 소유자의 Supabase Auth 사용자 UUID. 미설정 시 관련 도구는 비활성화됨 |
...
npm run source-twin:update # `source-twin` wire 호환 AST 소스 분석 갱신
